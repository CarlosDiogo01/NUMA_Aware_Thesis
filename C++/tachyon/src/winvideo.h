...
#include <crtdbg.h> //this code is added to support _CrtSetDbgFlag - to disable C Runtime from occasionally detecting memory issues.
...
bool WinInit(HINSTANCE hInstance, int nCmdShow, WNDCLASSEX *uwc, const char *title, bool fixedsize)
{
	...
	_CrtSetDbgFlag(_CRTDBG_CHECK_ALWAYS_DF); //This API is used to prevent the debug version of the CRT from "occasionally" detecting the same issues that Memory Error Checker Detects 
