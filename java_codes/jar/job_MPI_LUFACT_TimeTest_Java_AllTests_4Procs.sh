#PBS -N LUFACT_MPI_Hybrid_AllTests_4procs_TimeTest
...
#PBS -lnodes=compute-641-3:ppn=32:r641
...
alg="LUFACT_MPI_Hybrid"
exe="$HOME/NUMA_Aware_Thesis/java_codes/jar/lufact_hybrid.jar"
################################################################
...
Project_Folder="$HOME/NUMA_Aware_Thesis/java_codes/jar/${alg}_TimeTest_nprocs_${nprocs}_$node_info"
...
for size in $dataset
do
	echo "Size_${size}_${nprocs}_procs","$test1","$test2","$test3","$test4" >> "$TIMES_ALL_TESTS_PER_SIZE/TIMES_${alg}_Size_$size.csv"
	for thr in $thread_bundle
	do
	...
	#	median_test5=`cat "$test5/Size_$size/times.${alg}_$test5.$size.size.$thr.thr.csv" | awk 'FNR == '$index_val_to_collect' {print}'`
		echo "$thr.Threads","$median_test1","$median_test2","$median_test3","$median_test4" >> "$TIMES_ALL_TESTS_PER_SIZE/TIMES_${alg}_Size_$size.csv"
	done
done
