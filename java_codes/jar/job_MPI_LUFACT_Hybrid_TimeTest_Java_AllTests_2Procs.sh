#PBS -N LUFACT_MPI_Hybrid_AllTests_2procs_TimeTest
...
Project_Folder="$HOME/NUMA_Aware_Thesis/java_codes/jar/${alg}_TimeTest_${nprocs}_nprocs__aux_$node_info"
...
for size in $dataset
do
	echo "Size_${size}_${nprocs}_procs",$test6"" >> "$TIMES_ALL_TESTS_PER_SIZE/TIMES_${alg}_Size_$size.csv"
	for thr in $thread_bundle
	do
		...
		echo "$thr.Threads","$median_test6" >> "$TIMES_ALL_TESTS_PER_SIZE/TIMES_${alg}_Size_$size.csv"
	done
done
