...
echo "Using -XX:+UseNUMA -XX:+UseParallelGC flags"
mkdir $test2
for size in $dataset
do
...
echo "numactl --interleave=all ONLY"
mkdir $test3
for size in $dataset
do
...
echo "-XX:+UseNUMA -XX:UseParallelGC AND numactl --interleave=all"
mkdir $test4
for size in $dataset
do
...
echo "cpubind=0,1 (inteleave between CPU0 and CPU1) and membind=0 + JAVA NUMA Flags"
mkdir $test6
for size in $dataset
do
...
echo "Extract Medians and merge all times for all tests per thread"
