#PBS -N SOR_NUMA_AllTests_Java_AspectJ_DYNAMIC_SCHEDULE_NUMACTL_TimeTest
...
alg="SOR_AJ_DYNAMIC_SCHED"

#Tests with SM_lib_barrier STATIC schedule
exe1="SOR_AJ__SM_lib_barrier__for_dynamic.jar"
exe2="SOR_AJ__SM_used_defined_barrier__for_dynamic.jar"

path_exes="$HOME/NUMA_Aware_Thesis/java_codes/jar"
pack_exes="$exe1 $exe2"
################################################################
...
do
	...
	# For each exe, perform all tests
	mkdir $exe
	cd $exe	
	...
	do
		...
		do
			...
			do
				# Default execution
				$JAVA8 -jar $path_exes/$exe -5 $size $thr >> "$test1/Size_$size/$csv"
			done
	...
	do
		...
		do
			...
			do
				...
				$JAVA8 -jar -XX:+UseNUMA -XX:+UseParallelGC $path_exes/$exe -5 $size $thr >> "$test2/Size_$size/$csv"
			done
	...
	do
		...
		do
			...
			do
				...
				numactl --interleave=all $JAVA8 -jar $path_exes/$exe -5 $size $thr >> "$test3/Size_$size/$csv"
			done
	...
	do
		...
		do
			...
			do
				...
				numactl --interleave=all $JAVA8 -jar -XX:+UseNUMA -XX:+UseParallelGC $path_exes/$exe -5 $size $thr >> "$test4/Size_$size/$csv"
			done
	...
	do                      
		...
		do
			...
			do
				...
				numactl --cpubind=0,1 --membind=0 $JAVA8 -jar $path_exes/$exe -5 $size $thr >> "$test5/Size_$size/$csv"
			done
	...
	do
		...
		do
			...
			do      
				...
				numactl --cpubind=0,1 --membind=0 $JAVA8 -jar -XX:+UseNUMA -XX:+UseParallelGC $path_exes/$exe -5 $size $thr >> "$test6/Size_$size/$csv"
			done
