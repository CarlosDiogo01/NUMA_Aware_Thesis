#PBS -N SOR_NUMA_AllTests_Java_Flags_NUMACTL_TimeTest
...
Project_Folder="$HOME/NUMA_Aware_Thesis/java_codes/jar/${alg}_TimeTest_DEFAULT_$node_info"
...
for size in $dataset
do
	echo "Size_$size","$test1","$test2","$test3","$test4" >> "$TIMES_ALL_TESTS_PER_SIZE/TIMES_${alg}_Size_$size.csv"
	for thr in $thread_bundle
	do
