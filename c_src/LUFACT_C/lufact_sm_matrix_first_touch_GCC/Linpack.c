...
void run(const int size, const int validation, int numThreads)
{
 ...
 int JGFinitialise(Linpack *linpack, const int n, double a[n][n+1]){
...
double matgen (const int n, double a[n][n+1], double b[]){
...
int dgefa(const int n, double a[n][n+1], int ipvt[]){
    ...
    if (nm1 >= 0) 
    {
        for (int k = 0, kp1 = 1; k < nm1; k++, kp1++) 
        {
            ...
            // find l = pivot index
            const int l = ipvt[k] = idamax(n-k, col_k, k, 1) + k; 
...
void JGFvalidate(const int n, Linpack *linpack, double a[n][n+1]){
...
void dmxpy (int n1, double y[], int n2, int ldm, double x[], double m[n1][n1+1]){
...
void dgesl(int n, double a[n][n+1], int ipvt[], double b[], int job){
