...
public class Linpack {
	...
	double resid,time;
	double kf;
	int n,ntimes,info,lda,ldaa,kflops;
	int ipvt[];
  ...
  */
  public final int dgefa(double a[][], int lda, int n, int ipvt[])
  {
	  	final int nm1 = n - 1;
	  	double[] col_k;
	    int info = 0;
	    ...
	    if (nm1 >= 0) 
	    {
	    		for (int k = 0, kp1 = 1; k < nm1; k++, kp1++) 
	    		{
	    			...
	    			final int l = idamax(n-k, col_k, k, 1) + k; // find l = pivot index
	    			ipvt[k] = l;
	    			...
	    			if (col_k[l] != 0)
	    			{
	    				...
	    				parallelRowElimination(col_k, k, kp1, l);
	    			}
	    			else 
	    			{
	    				info = k;
 ...
 private void parallelRowElimination(final double[] col_k, final int k, final int kp1, final int l)
 {
	  final int slice = ((n-kp1) + totalThreads-1)/totalThreads;
	  for(int threadID = 1; threadID < totalThreads; threadID++) 
	  {
		  final int finalThreadID = threadID;
		  team.addTask(new Runnable()
		  {
			public void run() 
			{
				rowElimination(finalThreadID, slice, col_k, k, kp1, l);
				team.callBarrier();
			}
		}, threadID-1);
	  } 
	  rowElimination(0, slice, col_k, k, kp1, l);
	  team.callBarrier();
	  
 }

 	private void rowElimination(	final int threadID, final int slice,
								double[] col_k, int k, 
								int kp1, final int l) 
 	{
 		double[] col_j;
		...
		final int ilow  = threadID * slice + kp1;
		int iupper = ilow + slice;
 		
 		final int upper = (iupper > n) ? n : iupper;

 		for (int j = ilow; j < upper; j++)
 		{
 			col_j = a[j];
 			t = col_j[l];
 			if (l != k) 
  ...
  */
  ...
  /*
    ...
    jack dongarra, linpack, 3/11/78.
  */
  public final void dscal( int n, double da, double dx[], int dx_off, int incx)
  {
    int i,nincx;
